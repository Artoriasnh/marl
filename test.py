...
class PeartreeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Discrete(len(Actions_list) + 1)
    ...
    def update_time(self):
        if self.state < self.state_durations[self.state]:
    ...
    def step(self, action):
        if action == 0:  # all left to 1
            Rnochange(1).on()
            self.cur_action = Rnochange(1)
            if self.state_t == 10:
                R11(53).on()
                self.cur_action = R11(53)
                R12(27).on()
                self.cur_action = R12(27)
                R13(39).on()
                self.cur_action = R13(39)
                R14(31).on()
                self.cur_action = R14(31)
                R15(21).on()
                self.cur_action = R15(21)
                R16(13).on()
                ...
                print("set finish")
            elif self.state_t == 1000:
                R34(11).on()
                self.cur_action = R34(11)
            elif self.state_t == 1499:
                Rnochange(1).on()
                self.cur_action = Rnochange(1)
                pass
            print(self.pt_env)
        ...
        self.railway_1M99_time = [163, 190, 210]
                ...
                if t.isfinished:
                    ...
                    if len(t.statelist) == 0:
                        ...
                        if self.cur_action.srs == t.signaltrainstart and (self.cur_action not in t.troutelist):
                            ...
                            # 这次R的设置与t无关，并且车没在开
                            t.traintime = 0  # 车没在开
                            if self.state_t <= list(t.planroute.values())[0]:  # 如果车还没到预计的点
                                self.reward += abs(list(t.planroute.values())[0] - self.state_t)  # 现在都算是提前的所以没事
                            else:
                                self.reward = - 10  # 现在是delay了就直接done
                                print("delay GG!!!")
        ...
        return np.array(self.obs).astype(np.float32), self.reward, self.info, self.done
